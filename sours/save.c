...
void save(struct cell *head, struct cell *head2, struct energyblocks energyarr[10][10], int player_num, int turn, int cell1_num, int cell2_num, int n)
{
    ...
    fwrite(&player_num, sizeof(int), 1, fa);
    if (player_num == 2)
    {
        fwrite(&turn, sizeof(int), 1, fa);
        fwrite(&cell1_num, sizeof(int), 1, fa);
        fwrite(&cell2_num, sizeof(int), 1, fa);
        for (int i = 0; i < cell1_num; i++)
        {
            fwrite(&(current->x), sizeof(int), 1, fa);
            fwrite(&(current->y), sizeof(int), 1, fa);
            fwrite(&(current->energy), sizeof(int), 1, fa);
            current = current->next;
        }
        for (int j = 0; j < cell2_num; j++)
        {
            fwrite(&(current2->x), sizeof(int), 1, fa);
            fwrite(&(current2->y), sizeof(int), 1, fa);
            fwrite(&(current2->energy), sizeof(int), 1, fa);
            current2 = current2->next;
        }
    }
    ...
    for (int k = 0; k < n; k++)
    {
        for (int l = 0; l < n; l++)
        {
            if (energyarr[k][l].n == 0)
            {
                fwrite(&(energyarr[k][l].n), sizeof(int), 1, fa);
            }
            else if (energyarr[k][l].n == 1)
            {
                fwrite(&(energyarr[k][l].n), sizeof(int), 1, fa);
                fwrite(&(energyarr[k][l].energy), sizeof(int), 1, fa);
            }
        }
    }
    fclose(fa);
...
void playgame(struct cell *head, struct cell *head2, int n, struct energyblocks energyarr[10][10])
{
    while (1)
    {
        ...
        {
            save(head, NULL, energyarr, 1, 0, cellnum(head), 0, n);
...
void playgame3(struct cell *head, struct cell *head2, int n, struct energyblocks energyarr[10][10], int turn)
{
    while (1)
    {
        if (turn == 1)
        {
            ...
            {
                save(head, head2, energyarr, 2, 1, cellnum(head), cellnum(head2), n);
                printf("\t\t\t\t\tFile has been saved.\n");
            }
            else if (k == 5)
            {
